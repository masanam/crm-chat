...
class WebSocketController extends Controller implements MessageComponentInterface
{
  ...
  function onOpen(ConnectionInterface $conn)
  {
    \Illuminate\Support\Facades\Log::info('onOpen', [
      ...
      'conn' => compact('conn') + ['user_id' => null],
    ]);
  ...
  function onMessage(ConnectionInterface $conn, $msg)
  {
    ...
    \Illuminate\Support\Facades\Log::info('user_id', [
      'data' => $this->connections[$conn->resourceId]['user_id'],
    ]);
    if (is_null($this->connections[$conn->resourceId]['user_id'])) {
      ...
      foreach ($this->connections as $resourceId => &$connection) {
        \Illuminate\Support\Facades\Log::info('resourceId', [
          'data' => $resourceId,
        ]);
      ...
      $this->connections[$msg['to']]['conn']->send(
        json_encode([
          'msg' => $msg['content'],
