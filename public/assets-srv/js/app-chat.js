  ...
  (async function () {
    ...
    formSendMessage.addEventListener('submit', e => {
      ...
      if (messageInput.value) {
        ...
        var newData = JSON.stringify({
          phone: '+6285946916594', // replace with the actual phone number
          message: messageInput.value,
          user_id: userID.value
        });

        // Kirim data menggunakan metode push()
        var newRef = ref.push(newData);
        console.log('Data berhasil dikirim dengan ID:', newRef.key);
        ...
        fetch('https://crm.pasima.co/api/send-whatsapp', {
          ...
          body: JSON.stringify({
            phone: '+' + waTo, // replace with the actual phone number
            message: messageInput.value,
            user_id: userID.value
          })
        })
    ...
    async function fetchChatData() {
      if (waTo) {
        ...
        const endpoint = 'https://crm.pasima.co/api/get-chats/' + waTo;
        ...
        fetch(endpoint)
          ...
          .then(data => {
            ...
            data.forEach(message => {
              ...
              console.log('check wa from: ' + waFrom);
              if (message.from === waFrom) {
                newMessage.className = 'chat-message chat-message-right';
                newMessage.innerHTML = `
              <div class="d-flex overflow-hidden">
            ...
            `;
              ...
              <div class="d-flex overflow-hidden">
                  <div class="user-avatar flex-shrink-0 me-2">
                      <div class="avatar"><span class="avatar-initial rounded-circle bg-label-secondary"><i class='ti ti-user'></i></span></div>
                  </div>
                  <div class="chat-message-wrapper flex-grow-1">
                      <div class="chat-message-text">
                          <p class="mb-0">${message.message}</p>
