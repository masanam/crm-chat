 ...
 if (messenger != 0) {
   ...
   $.ajax({
     ...
     success: (data) => {
       ...
       $(".messenger-infoView .delete-conversation").hide();
...
channel.bind("messaging", function (data) {
 if (data.from_id == getMessengerId() && data.to_id == auth_id) {
   ...
   $(".chat-contact-list-item[data-contact=" + getMessengerId() + "]")
     .find("tr>td>b")
     .remove();
 }
...
//  getFavoritesList();
 ...
 $("body").on("click", ".group-list-item", function () {
   ...
   $(".group-list-item").removeClass("m-list-active");
   $(this).addClass("m-list-active");
   const groupId = $(this).attr("data-contact");
   routerPush(document.title, `${url}/groups/${groupId}`);
