    ...
    <div class="sidebar-body">
        ...
        <div class="container pt-2">
            <div class="d-flex align-items-center justify-content-between">
                <img class="text-dark" src="{{asset('assets/svg/icons/company-icon.svg')}}" alt="info">
                <h4 class="pt-3 fw-bolder text-dark float-left">{{ $title }}</h4>
                @if($targetOpenModal)
            ...
            <ul class="nav nav-tabs" id="chats-tabs" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link nav-item-custom {{ $key === 0 ? 'active' : '' }}" id="{{ strtolower($value->name) }}-tab" data-bs-toggle="tab" data-bs-target="#{{ strtolower($value->name) }}" type="button" role="tab" aria-controls="{{ strtolower($value->name) }}" aria-selected="true">{{ $value->name }} ({{ $value->lengthData }})</button>
                </li>
                @endforeach
            </ul>
            @endif
