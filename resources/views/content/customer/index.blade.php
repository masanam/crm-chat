...
<div class="row">
    <div class="">
        ...
        <div class="app-chat customer overflow-hidden">
            <x-page-title title="Customers" placeholderSearchText="Search leads"
                targetOpenModal="#customers"></x-page-title>
            <div class="row g-0">
...
<x-modal title="Add New Customer" name="add-customer" submitText="Submit" isModalStack="{{ true }}"
    targetNameModalStack="customers" modalClass="" buttonSubmitClass="" isPost="true"
    ...
    <div class="d-flex flex-column gap-3">
        <div class="d-flex flex-column gap-2">
            ...
            <div class="d-flex flex-column gap-3">
                <x-input-floating label="Customer Name" id="customer-name" name="client_name"></x-input-floating>
        ...
        <div class="d-flex flex-column gap-2 border-bottom border-1 pb-3 align-items-start">
            ...
            <div class="d-flex flex-column align-items-start gap-3 w-100">
                <div class="d-flex justify-content-between gap-5 w-100">
                    <x-input-floating label="First Name" id="first-name" name="first_name">
                    </x-input-floating>
                    <x-input-floating label="Last Name" id="last-name" name="last_name">
                    </x-input-floating>
                </div>
                <div class="d-flex justify-content-between gap-5 w-100">
                    <x-input-floating label="Whatsapp" id="whatsapp" name="whatsapp">
                    </x-input-floating>
                    <x-input-floating label="No Whatsapp" id="phone_number" name="phone_number">
                    </x-input-floating>
                </div>
        ...
        <div class="d-flex flex-column gap-2">
            ...
            <div class="d-flex flex-column gap-3 w-100">
                <x-input-floating label="Deal Revenue" id="deal revenue" name="deal revenue"></x-input-floating>
                <div class="d-flex justify-content-between gap-5 w-100">
                    <x-input-floating label="Close Date" id="flatpickr-date" name="flatpickr-date">
                    </x-input-floating>
                    <x-input-floating label="Next Step" id="next step" name="next step">
                    </x-input-floating>
                </div>
                <x-input-floating label="Description" id="description" name="description"></x-input-floating>
            </div>
        </div>
    </div>
</x-modal>
...
<x-modal title="Customers" name="customers" wrapperModalClass="modal-right" isUsingBtnFooter="{{ false }}">
    <div class="d-flex flex-column gap-3 modal-add-contact">
        <div class="d-flex flex-column gap-2 border-bottom">
            <div class="">
                <span style="position: absolute; margin-top: 18px; margin-left: 12px;" id="basic-addon-search31"><i
                        class="ti ti-search"></i></span>
                <input style="padding: 0.422rem 2.2rem; border-radius: 20px;" type="text" class="form-control"
                ...
                <div class="d-flex flex-column">
                    <div data-bs-dismiss="modal" class="d-flex align-items-center gap-2 modal-contact-body">
                        ...
                        <div class="">
                            ...
                            <div class="d-flex align-items-center gap-2 cursor-pointer chat-contact-list-item p-2"
                                data-contact="{{ $item->phone_number }}">
                                <div class="flex-shrink-0 avatar">
                                    <span class="avatar-initial rounded-8 bg-label-success text-dark">{{
                                        Helper::getInitial($item->client_name); }}</span>
                                </div>
                                <h6 class="text-dark contact-title" style="padding:3px;border-bottom:1px solid #999;"
                                    data-contact="{{ $item->phone_number }}">
                                    <h6 class="chat-contact-name text-truncate m-0 text-dark fw-bolder"
                                        data-id="{{ $item->phone_number }}" data-counter="{{ $item->created_at }}"
                                        data-contact="{{ $item->client_name }}" data-type="contact"
                                        data-job="{{ $item->title }}" data-company="{{ $item->company_name }}"
                                        data-closed="{{ $item->closed_date }}" data-budget="{{ $item->budget }}">
...
<x-modal title="Add Contact" name="add-edit-contact" submitText="Save contact" isPost="true"
    url="{{ route('leads.change') }}" buttonSubmitClass=""
    buttonWrapperSubmitClass="d-flex justify-content-center align-items-center w-100">
    <div class="d-flex flex-column gap-2 border-bottom border-1 pb-3 align-items-start">
        <div class="d-flex flex-column gap-3">
            <div class="d-flex justify-content-between gap-5 w-100">
                <x-input-floating label="First Name" id="first-name" name="first_name" class="client-name">
                </x-input-floating>
                <x-input-floating label="Last Name" id="last_name" name="last_name">
                </x-input-floating>
            </div>
            <x-input-floating label="Job Title" id="client-job" name="job_title">
            </x-input-floating>
            <div class="d-flex justify-content-between gap-5 w-100">
                <x-input-floating label="Channel" placeholder="Please select channel" id="channel" name="channel"
                    type="select" :options="$listChannels">
                </x-input-floating>
...
{{-- modal filter --}}
<x-modal title="Filter" name="filter" submitText="Apply" buttonSubmitClass=""
    buttonWrapperSubmitClass="d-flex justify-content-center align-items-center w-100"
    isUsingBtnFooterClose="{{ true }}">
    <div class="d-flex flex-column gap-5">
        <div class="d-flex justify-content-between gap-5 w-100">
            <x-input-floating label="Start Date" id="start-date" name="start-date">
            </x-input-floating>
            <x-input-floating label="End Date" id="end-date" name="end-date">
            </x-input-floating>
        </div>
        <x-input-floating label="Status" id="status" name="status" type="select" :options="$status">
        </x-input-floating>
        <x-input-floating label="Quality" id="quality" name="quality" type="select" :options="$quality">
        </x-input-floating>
        <x-input-floating label="Stage" id="stage" name="stage" type="select" :options="$stages">
        </x-input-floating>
    </div>
</x-modal>

{{-- modal template --}}
<x-modal title="Template" name="template-chat" submitText="Send Template" isPost="true"
    url="{{ route('send.template') }}" buttonSubmitClass=""
    buttonWrapperSubmitClass="d-flex justify-content-center align-items-center w-100">
    <div class="d-flex flex-column gap-5">
    ...
    <div class="d-flex align-items-center justify-content-between mt-5">
        ...
        <button type="button" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#create-new-template"
            style="border: none; background: transparent; color: #33B6B9;">+ create new template</button>
        <!-- <button type="button" data-bs-dismiss="modal" class="btn btn-primary">Send</button> -->
    </div>
</x-modal>

{{-- modal create new template --}}
<x-modal title="Create New Template" name="create-new-template" buttonSubmitClass="" modalClass=""
    buttonWrapperSubmitClass="d-flex justify-content-center align-items-center w-100" isUsingBtnFooter="{{ false }}">
    <div class="d-flex flex-column gap-3">
        <div class="d-flex flex-column gap-4 w-100">
            <x-input-floating label="Title" id="title" name="title">
            </x-input-floating>
            <x-input-floating label="Write template here" placeholder="" id="template" name="template" type="textarea">
            </x-input-floating>
        </div>
...
background: #FFF;">
                        <div class="form-group ">
                            <form id="update-status" method="POST" action="{{ route('customers.updateStatus') }}"
                                ...
                                <div class="modal-footer d-flex justify-content-center align-items-center w-100 p-4">
                                    <button type="submit" data-bs-dismiss="modal" id="update-status"
